function tree = growtrees_2pix(data,param)
...
cnt_total = 1;
[labels,~] = unique(data(:,end)); %%% The end index is the data label
for T = 1:param.num % Number of trees generated
    ...
    prior = histc(data(idx,end),labels)/length(idx); % Histogram counting
    
    % Initialise base node
    tree(T).node(1) = struct('idx',idx,'t',nan,'dim',-1,'prob',[]);
    
    % Split Nodes are created here for the corresponding tree
    for n = 1:2^(param.depth-1)-1
        [tree(T).node(n),tree(T).node(n*2),tree(T).node(n*2+1)] = splitNode_2pix(data,tree(T).node(n),param);
    end
    
    % Leaf Nodes created after split nodes reached a certain depth
    cnt = 1;
