...
switch MODE
    ...
    case 'Caltech' % Caltech dataset
        ...
        [codeIndex,codebook] = kmeans(desc_sel', numBins);
        ...
        % Bag of words representation for training and testing sets
        BOW_tr = cell(size(desc_tr));
  
        for i=1:length(classList)
            for j=1:imgSel(1)
                desc_size = size(desc_tr{i,j});
                BOW_tr{i,j} = zeros(1,numBins);
                for k=1:desc_size(2)
...
%         end
        ...
        BOW_te = cell(size(desc_te));
        
        for i=1:length(classList)
            for j=1:imgSel(2)
                desc_size = size(desc_te{i,j});
                BOW_te{i,j} = zeros(1,numBins);
                for k=1:desc_size(2)
