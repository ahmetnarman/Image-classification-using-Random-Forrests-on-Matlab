...
for n = 1:iter
    ...
    dim1 = randi(D-1); % Pick one random dimension
    dim2 = randi(D-1); % Pick one random dimension
    % Split function - Modify here and try other types of split function
    while dim1==dim2
        dim1 = randi(D-1); % Pick one random dimension
    end 
    dim=[dim1 dim2];
    diff_data=data(:,dim1)-data(:,dim2);
    d_min = single(min(diff_data(:,1))) + eps; % Find the data range of this dimension
